...
gustaverow = Tunnel.getInterface("gustaverow_modoadm")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARS
-----------------------------------------------------------------------------------------------------------------------------------------
local cDistance = 150
...
RegisterCommand("am",function()
	...
	if showIds then
		gustaverow.reportLog("ADMIN MODE ON")
		print("ADMIN MODE ON")
	else
		gustaverow.reportLog("ADMIN MODE OFF")
...
-- ANTISIXCOMMAND
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("antigustaverowlog",function()
	if not admin then
		return
	end
...
Citizen.CreateThread(function()
	while true do
	    for _, id in ipairs(GetActivePlayers()) do
			...
			local pid = gustaverow.getId(GetPlayerServerId(id))
			if pid == -1 then
				return
			end
